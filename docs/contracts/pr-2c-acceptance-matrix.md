...
| FMP corrections | Same record tuple plus changed semantic body/site has a new deterministic revision | FMP fixture agent |
...
| NVIDIA revisions | Same non-URL family plus changed retained projection or raw HTML bytes yields a new revision; intra-feed conflict quarantines | NVIDIA fixture agent |
...
| URL-only raw change | Semantic projection stays equal but changed raw primary digest creates a new revision/draft accepted by trusted capture | NVIDIA fixture agent |
...
| V1 identical mirrors | FMP/IR V1 with equal raw primary digest and both null bundle hashes may emit mirror-duplicate while preserving provider provenance | Cross-source agent |

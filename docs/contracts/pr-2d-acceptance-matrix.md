...
| `AM-ID-010` | provider-source, study, fixture | After `AM-ID-012` validates the primitive issuer/instrument family, the exact `scc1_` preimage binds both recomputed `releaseClusterKey` and `releaseKind` in addition to representative schedule evidence. Literal one-field mutation/collision vectors prove a different `scc1_` for quarterly versus annual releases; two distinct non-null cross-source release keys; two distinct native schedule IDs; fiscal-period versus native-date cluster bases; and multiple distinct schedule items carried by one identical `scheduleSourceObservationId`. Every vector recomputes and cross-validates `releaseKind`, `plannedFiscalPeriod`, selected `clusterBasis`, `releaseClusterKey`, representative source/native evidence, and candidate fields; caller-supplied mismatches reject before hashing. | literal preimage bytes/hashes, former collision witness, and one-field mutation matrix | `REQUIRED_PENDING` |
| `AM-ID-011` | provider-source, study, fixture | Each selected `scc1_` mutation from the seven-case family propagates deterministically through the containing `sfs1_`, selected `scl1_`, `sfm1_`, and resulting eligible `sdf1_` primitive preimages and changes every dependent displayed ID while unrelated identities remain stable. Two semantically distinct clusters in one frame coexist without `study.duplicate-cluster`; only byte-identical duplicate candidate identity or same-ID/conflicting-preimage evidence is fatal. Frame/member permutation, source packaging, restart, and backend cannot collapse the repaired identities or create a false duplicate. | downstream `sfs1_`/`scl1_`/`sfm1_`/`sdf1_` golden differential and no-false-duplicate vectors | `REQUIRED_PENDING` |
| `AM-ID-012` | observation ledger, provider-source, study, fixture | Every one of the exactly seven `SCC-*` literal cases carries the complete original-synthetic issuer-mapping primitive preimage and complete PR 2D instrument primitive preimage. The validator first recomputes inherited `imap1_` with the frozen observation-ledger derivation and direct `min1_` with the accepted instrument derivation, byte-compares both displayed IDs, and only then derives `releaseClusterKey` or `scc1_`. Wrong-family `ism1_`/`ins1_`, syntactically valid but forged `imap1_`/`min1_`, missing/extra primitive fields, mismatched displayed IDs, or invalid issuer-to-instrument linkage reject before cluster-key hashing and emit no candidate or descendant ID. From the validated identities, literal expectations independently recompute all seven raw release-cluster keys, all seven eight-field `scc1_` IDs, the retired six-field collision witness, and every selected downstream `sfs1_`/`scl1_`/`sfm1_`/`sdf1_` descendant. | primitive `imap1_`/`min1_` golden and forgery corpus plus corrected seven-case SCC/retired-collision/descendant literals | `REQUIRED_PENDING` |

...
export class DefaultArtifactRetentionController implements ArtifactRetentionController {
  ...
  registerOwnership(input: Omit<RetentionOwnership, "ownershipId">): RetentionOwnership {
    ...
    this.#journal.registerOwnership(value);
    return value;
  }
  ...
  async enforceStop(input: Omit<RetentionStopEvent, "stopEventId">): Promise<RetentionReceipt> {
    ...
    const derivedIds = sortedUnique(ownership.flatMap((value) => value.derivedIds));

    // Stop and all use denials are one durable journal operation and precede any erasure work.
    this.#journal.recordStopAndDenials(stop, derivedIds);
    ...
    const planBody = {
      ...
      datasetIds: sortedUnique(ownership.map((value) => value.datasetId)),
      feedIds: sortedUnique(ownership.map((value) => value.feedId)),
      endpointChannelIds: sortedUnique(ownership.map((value) => value.endpointChannelId)),
      artifactObservationIds: sortedUnique(ownership.map((value) => value.artifactObservationId)),
      artifactDigests: sortedUnique(ownership.map((value) => value.artifactDigest)),
      derivedIds,
    ...
    const receiptBody = {
      ...
      attemptCount:
        attemptCount +
        plan.artifactDigests.reduce(
          (sum, digest) => sum + this.#journal.attemptsFor(plan.planId, digest).length,
          0,
        ),
  ...
  assertArtifactUseAllowed(digest: string): void {
    ...
    if (this.#journal.digestUseDenied(digest))
